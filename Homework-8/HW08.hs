...
transposition = Conj left_right right_left
                where left_right = modus_tollens
                      right_left nqnp p = not_not_p $ modus_tollens nqnp (p_not_not p)

not_not_p :: Not (Not p) -> p
not_not_p nnp = case excluded_middle of
    Left p  -> p
    Right q -> absurd (nnp q)

p_not_not :: p -> Not (Not p)
p_not_not p = \np -> np p
...
de_morgan = Conj left_right right_left
    where left_right npq = case p_all of
              Left  p  -> absurd $ npq (Left p)
              Right np -> case q_all of
                  Left  q  -> absurd $ npq (Right q)
                  Right nq -> Conj np nq

          right_left (Conj np nq) p_or_q = nq $ disjunctive_syllogism p_or_q np
